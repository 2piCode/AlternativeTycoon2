...
namespace Game 
{
    public class ProjectNote : MonoBehaviour
    {
        ...
        private void Start()
        {
            GameManager.singleton.onFinishDay += writeEventsInNote;
        ...
        private void writeEventInNote(IEvent ev)
        {
            string eventText = $"{ev.name}\n{ev.description}\n\n";
            writeTextInNote(eventText);
        }

        private void writeEventsInNote()
        {
            Debug.Log("write");
            foreach (var item in GameManager.singleton.eventsOnDay)
            {
                writeEventInNote(item);
            }
        }
    }
}